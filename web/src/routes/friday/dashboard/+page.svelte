<script lang="ts">
	...
	import { parsePythonDate } from '$lib/utils/date';
	...
	const summaryCards = $derived([
		...
		{
			label: 'Saldo do Mês',
			value: fmtCurrency(data.summary.balance),
			sub: 'Receitas − Gastos',
			icon: CreditCardIcon,
			color: data.summary.balance >= 0 ? 'text-friday-orange' : 'text-failed',
			bg:
				data.summary.balance >= 0
					? 'bg-friday-orange/10 border-friday-orange/20'
					: 'bg-failed/10 border-failed/20'
		}
	]);
	...
	const quickLinks = [
		{
			label: 'Contas',
			icon: WalletIcon,
			route: '/friday/accounts',
			desc: 'Gerencie suas contas e cartões'
		},
		{
			label: 'Atributos',
			icon: TagIcon,
			route: '/friday/transaction-attributes',
			desc: 'Tags, métodos de pagamento e moedas'
		}
	];
...
<div class="flex flex-col gap-6 text-white">
		...
		{#each summaryCards as card}
			<div class="rounded-xl border {card.bg} p-5">
				<div class="mb-3 flex items-center justify-between">
					<span class="text-xs font-medium uppercase tracking-widest text-white/40"
						>{card.label}</span
					>
	...
	<!-- Middle row: Quick links + Recent transactions -->
	<div class="grid grid-cols-3 gap-4">
		<!-- Quick links -->
		<div class="col-span-1 flex flex-col gap-3">
			<h2 class="text-sm font-semibold uppercase tracking-wider text-white/60">Atalhos</h2>
			{#each quickLinks as link}
				...
				>
					<div
						class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/5 transition-colors group-hover:border-friday-blue/30 group-hover:bg-friday-blue/10"
					>
						<link.icon
							size={16}
							class="text-white/50 transition-colors group-hover:text-friday-blue"
						/>
					</div>
					...
					<ArrowRightIcon
						size={14}
						class="shrink-0 text-white/20 transition-all group-hover:translate-x-0.5 group-hover:text-friday-blue"
					/>
				</a>
			{/each}
		</div>
		...
		<div class="col-span-2 rounded-xl border border-white/8 bg-white/[0.03] p-6">
			<h2 class="mb-4 text-sm font-semibold uppercase tracking-wider text-white/60">
				Transações Recentes
			</h2>
			{#if data.recentTransactions.length > 0}
				<div class="flex flex-col gap-2">
					{#each data.recentTransactions as tx}
						{@const parsed = tx.dateTransaction ? parsePythonDate(tx.dateTransaction) : null}
						<div
							class="flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] px-4 py-3"
						>
							<div class="flex w-10 shrink-0 flex-col items-center">
								{#if parsed}
									<span class="text-lg font-bold text-white/70">{parsed.dia}</span>
									<span class="text-[10px] uppercase text-white/30">{parsed.mesAno}</span>
								{/if}
							</div>
							<div class="min-w-0 flex-1">
								<p class="truncate text-sm text-white/80">
									{tx.description || tx.subcategoryLabel || tx.categoryLabel}
								</p>
								<p class="text-xs text-white/30">
									{tx.categoryLabel}{tx.subcategoryLabel ? ` / ${tx.subcategoryLabel}` : ''}
								</p>
							</div>
							<span
								class="shrink-0 text-sm font-semibold {tx.categoryType === 'income'
									? 'text-success'
									: 'text-failed'}"
							>
								{tx.categoryType === 'income' ? '+' : '-'}{fmtCurrency(tx.value)}
							</span>
						</div>
					{/each}
				</div>
			{:else}
				<div class="flex flex-col items-center justify-center gap-3 py-10 text-center">
					...
					<p class="text-xs text-white/20">
						Adicione uma conta e registre sua primeira transação
					</p>
					<a
	...
	<!-- Bottom: Spending by category -->
	<div class="rounded-xl border border-white/8 bg-white/[0.03] p-6">
		<div class="mb-4 flex items-center justify-between">
			<h2 class="text-sm font-semibold uppercase tracking-wider text-white/60">
		...
		{#if data.topCategories.length > 0}
			<div class="flex flex-col gap-3">
				{#each data.topCategories as cat, i}
					<div class="flex items-center gap-4">
						...
						<span class="w-16 shrink-0 text-right text-xs text-white/40">
